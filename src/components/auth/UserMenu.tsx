...
export default function UserMenuFixed() {
  ...
  const getTierBadge = () => {
    ...
    if (user.ownedChapters.length > 0) return `💎 ${user.ownedChapters.length} Chapters`
    return '📖 Free'
  }
  ...
  return (
    <div className="fixed top-6 right-6 z-50" ref={menuRef}>
      ...
      >
        <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getTierColor()} flex items-center justify-center ring-2 ring-offset-2 ring-offset-transparent ring-transparent group-hover:ring-neutral-700 transition-all shadow-lg`}>
        ...
        <div className="absolute right-0 mt-3 w-72 bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="p-4 border-b border-neutral-800 bg-gradient-to-br from-neutral-900 to-neutral-800">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getTierColor()} flex items-center justify-center`}>
            ...
            <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full bg-black/40 text-xs font-ui uppercase tracking-wider ${getTierTextColor()}`}>
              {getTierBadge()}
            </div>
          </div>
